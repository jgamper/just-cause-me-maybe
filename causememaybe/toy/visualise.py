...
def plot_treatment(dgp: DataGeneratingProcess):
    ...
    if dgp.overlap != "random":
        h=0.2
        x_min, x_max = dgp.X[:, 0].min() - .5, dgp.X[:, 0].max() + .5
        y_min, y_max = dgp.X[:, 1].min() - .5, dgp.X[:, 1].max() + .5
        xx, yy = np.meshgrid(np.arange(x_min, x_max, h),
                             np.arange(y_min, y_max, h))
        Z = dgp.clf.predict_proba(np.c_[xx.ravel(), yy.ravel()])[:, 1]
        cm = plt.cm.RdBu
        Z = Z.reshape(xx.shape)
        pcm = ax1.contourf(xx, yy, Z, cmap=cm, alpha=.8)
    ...
    for i in range(sample_size):
        ...
        ax1.scatter(dgp.X[i, 0], dgp.X[i, 1], marker=m, alpha=0.8, c=c, edgecolors='black', s=80);
